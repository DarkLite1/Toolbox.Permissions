...
Function Get-PermissionMatrixGroupMembersHC {
    <#
    .SYNOPSIS
        List all AD objects and its members from the permission matrix.

    .DESCRIPTION
        Generate a list of all AD objects used in the permission matrix and 
        check if the AD objects exist and if they have members when it's a
        group.
        When run without a 'Path' the correct permission matrix file will be
        requested by using 'Out-GridView' on the 'SelfHelp'.

    .PARAMETER Path
        The path to the Excel file.

    .PARAMETER GroupNamesOnly
        Only generate the AD object names and don't check group members.

    .EXAMPLE
        Get-PermissionMatrixGroupMembersHC -Path 'Star-Trek.xlsx'
    ...
    .EXAMPLE
        ...
        Get-PermissionMatrixGroupMembersHC @params
    ...
    Param (
        ...
        [String]$Path,
        [String]$SelfHelp = '\\unite.grouphc.net\benelux\staffdepartments\it_bene\selfhelp\Documents\Folders - Permission matrix',
        [Switch]$GroupNamesOnly
    )

    Begin {
        if (-not $Path) {
            $File = Get-ChildItem -Path $SelfHelp -Recurse -File | Select-Object FullName, LastWriteTime |
            Sort-Object LastWriteTime -Descending |
            Out-GridView -PassThru -Title 'Select the permission matrix' |
            Select-Object -ExpandProperty FullName

            if (-not $File) {
                throw 'No permission matrix selected'
            }
        }
        else {
            $File = $Path
        ...
        Function Get-GroupNames {
            <#
            .SYNOPSIS
                Generates the correct active directory object names that are 
                used in the permission matrix.

            .DESCRIPTION
                Generates the correct active directory object names that are 
                used in the permission matrix based on the input for the 
                $GroupName and the $SiteName in the 'Settings' tab page of the 
                Excel sheet.

            .PARAMETER Matrix
                The RAW permission matrix array for which we will generated the 
                AD Object names.

            .PARAMETER GroupName
                The active directory ID-String

            .PARAMETER SiteCode
                The abbreviated name of the site.

            .PARAMETER Status
                The status of the matrix, if it's 'Enabled' or not
            #>
            ...
            Process {
                $Matrix | Get-Member | Where-Object { ($_.MemberType -EQ 'NoteProperty') -and
                        ($_.Name -ne ($Matrix[0].PSObject.Properties).Name[0]) } | ForEach-Object {

                    $Name = $_.Name
                    $Value = $Matrix.$Name

                    $GroupResult = Switch ($Value[1]) {
                        'GroupName' { $GroupName + ' ' }
                        '' { break }
                        Default { $Value[1] + ' ' }
                    }

                    $SiteResult = Switch ($Value[0]) {
                        'SiteCode' { $SiteCode + ' ' }
                        '' { break }
                        Default { $Value[0] + ' ' }
                    }

                    $Result = ($GroupResult + $SiteResult + $Name).Trim()
        ...
        Try {
            $Permissions = Import-Excel -Path $File -Sheet 'Permissions'
            $Settings = Import-Excel -Path $File -Sheet 'Settings'
        }
        Catch {
    ...
    Process {
        $Groups = Foreach ($S in $Settings) {
            if ($S.SiteCode -and $S.GroupName) {
                $GroupParams = @{
                    Matrix    = $Permissions
                    SiteCode  = $S.SiteCode
                ...
                Get-GroupNames @GroupParams
            }
        }

        if ($GroupNamesOnly) {
            $Groups | Group-Object ADObject | Select-Object -ExpandProperty Name
            break
        }

        Foreach ($G in ($Groups | Group-Object ADObject | Select-Object -ExpandProperty Name)) {

            if (((Get-ADObject -Filter { Name -eq $G }).ObjectClass -eq 'Group')) {
                $Users = Get-ADGroupMember $G -Recursive | Select-Object -ExpandProperty Name

                foreach ($U in $Users) {
                    Write-Verbose "Name '$G' - Type 'Group' - User '$U'"
                    [PSCustomObject]@{
                        Name   = $G
                        Type   = 'Group'
                        InAD   = $true
            ...
            elseif (((Get-ADObject -Filter { Name -eq $G }).ObjectClass -eq 'User')) {
                Write-Verbose "Name '$G' - Type 'User'"
                [PSCustomObject]@{
                    Name   = $G
                    Type   = 'User'
                    InAD   = $true
                ...
                Write-Verbose "Name '$G' - Not in AD"
                [PSCustomObject]@{
                    Name   = $G
                    Type   = $null
                    InAD   = $false
